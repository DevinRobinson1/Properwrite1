...
class ComprehensiveValuationService:
            ...
            if search_response.status_code == 200:
                ...
                if isinstance(search_data, list) and len(search_data) > 0:
                    ...
                    if zpid:
                        ...
                        # Try to get detailed property information (paid tier)
                        try:
                            ...
                            if details_response.status_code == 200:
                                ...
                                # Check if we have detailed data (paid tier)
                                if details_data and not details_data.get('message'):
                                    ...
                                    # Extract tax history for recent value
                                    ...
                                    valuation_data['zillow_details'] = {
                                        ...
                                        'tax_history': tax_history[:3] if tax_history else [],
                                        'hoa_fee': details_data.get('monthlyHoaFee')
                                    }
                                    ...
                                    # Free tier - property found but details require subscription
                                    logging.info(f"Zillow found property but details require subscription upgrade")
                                    valuation_data['zillow_property_found'] = {
                                        'zpid': zpid,
                                        'address': matched_address,
                                        'subscription_note': 'Property found - detailed valuations require Zillow+RapidAPI subscription upgrade'
                                    }
                                    valuation_data['sources_used'].append('Zillow (search only)')
                                    return
                            else:
                                logging.warning(f"Zillow property details API failed: {details_response.status_code}")
                                
                        except Exception as e:
                            logging.warning(f"Error accessing Zillow property details: {e}")
                            
                        # Fallback - indicate property was found but details unavailable
                        valuation_data['zillow_property_found'] = {
                            'zpid': zpid,
                            'address': matched_address,
                        ...
                        valuation_data['sources_used'].append('Zillow (search only)')
                        logging.info(f"Zillow: Property found but detailed valuation unavailable")
                    else:
                ...
                # Handle legacy response format
                    ...
                    # Handle legacy format (not common anymore)
                    
                else:
                    logging.warning("No Zillow matches found")
    ...
    def _cleanup_legacy_zillow_code(self):
